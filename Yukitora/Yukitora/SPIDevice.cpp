...
unsigned char SPIDevice::readRegister(unsigned int registerAddress){
	unsigned char send[2], receive[4];
	memset(send, 0, sizeof send);
	memset(receive, 0, sizeof receive);
	send[0] = (unsigned char) (0x00);
...
// unsigned char* SPIDevice::readADC(){
// 	uint8_t tx[] = {0x00, 0x00, 0x00, 0x00,	};
// 	uint32_t rx[ARRAY_SIZE(tx)] = {0, };
// }

unsigned char* SPIDevice::read(){
	unsigned char* data = new unsigned char[3];
	unsigned char send[3], receive[3];
	memset(send, 0, sizeof send);
	memset(receive, 0, sizeof receive);
	send[0] = (unsigned char) (0x01);
	// send[0] = (unsigned char) (0x80 | registerAddress);
	this->transfer(send, receive, 3);
	//cout << "The value that was received is: " << (int) receive[1] << endl;
	memcpy(data, receive+1, 3);  //ignore the first (address) byte in the array returned
	return data;
}
